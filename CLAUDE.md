...
- **`@discord/arborium-rt`** — the TypeScript runtime wrapper. Ships
  ...
  --release`, which emits `arborium_rt_wasm.wasm`; `package host` copies it
  under the historical published filename), the
  ...
  `package grammars`; the host + runtime wasms are staged by `package host`.
...
./scripts/arborium-rt build grammar group-acorn json
...
# Pack the runtime package tarball (after build + package host + package
# grammars). Releases ship this tarball via GitHub Releases, not npm.
...
- **Staging** lives in TS (the `buildNodeGrammars` step in
  `cli/src/commands/build/node.ts`), reusing the same `tree-sitter generate`
  + flatten + source-staging helpers as the wasm build — extracted into
  `cli/src/lib/stage-grammar.ts` and shared with `cli/src/commands/build/grammar.ts`.
- **Build invocation** (`cli/src/commands/build/node.ts`): `cargo build --release -p
...
`arborium-rt package host`, and the TS resolver uses
...
auto `fs`-based loader; see the `build host` notes below).
...
`write-grammars-index.ts` whenever `arborium-rt package grammars`
runs; `package host` + at least one `package grammars` run is the
...
table); each subcommand lives under `src/commands/` (namespaced as
`build/*.ts`, `package/*.ts`, etc.). Shared helpers in `src/lib/util.ts`
...
`ARBORIUM_RT_ROOT`). The `build host` module (`src/commands/build/host.ts`)
...
regenerates the wasm package's `src/grammars.ts` after `package grammars`;
...
- `target/wasm32-unknown-emscripten/release/arborium_rt_wasm.wasm`
  ...
  `arborium-rt-wasm` cdylib. `package host` copies it into the runtime
...
- `packages/arborium-rt/dist/grammars/<lang>/` — per-grammar assets
  (the wasm + flattened `.scm` files). Written by `package grammars`;
...
- `packages/arborium-rt/dist/grammars.js` — single `GRAMMARS` aggregator.
  ...
  `write-grammars-index.ts` after `package grammars`. Exposed to
...
- **Adding a wasm `extern`** (e.g., pulling in a new libc or tree-sitter
  ...
  in `cli/src/commands/build/host.ts` and rebuilding the host.
